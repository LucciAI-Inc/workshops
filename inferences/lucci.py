...
def chat_completion_request(messages, model=MODEL_ID):
    headers = {
        "Content-Type": "application/json",
        "Authorization": "Bearer " + os.getenv('LUCCI_API_KEY'),
    }
    json_data = {
        "model": model, 
        "messages": messages, 
    ...
    try:
        response = requests.post(
            "https://api.lucci.ai/v1/chat/completions",
            headers=headers,
            json=json_data,
        )
        return response
